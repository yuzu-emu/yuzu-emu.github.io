...
Hi yuz-ers! Work goes on, and we finished March with improvements in performance, graphics, audio, CPU precision, input, and much more!
...
This requires some of backtracking so let’s rewind a little bit.
...
A thread called `HostTiming` is used then to process HLE events such as input and audio.
...
During that 0.67ms, the CPU thread is not doing anything and consumes power without actually working.

Here comes the benefit of removing support for Windows 7 and 8.x. 
...
The new free resources mean better performance for low core count CPUs like quad cores, and for systems with limited power budgets like laptops and handhelds.
...
You can notice it as inaccurate match times.
...
100000 cycles was the magic number in Morph’s testing, which reduced CPU use/power consumption up to 20%, on top of the previous benefits.
...
Oh well, 12th/13th Gen users running actual working GPU drivers from NVIDIA or AMD will benefit from all this work greatly.
...
Some trial and error, and some big help from the great [bylaws](https://github.com/bylaws), from [Skyline](https://github.com/skyline-emu/skyline) emulator, and Maide managed to sort out the issues, providing {{< gh-hovercard "9896" "accurate colour rendering" >}} without breaking other games.
...
This means that OpenGL users, especially those with Fermi and Kepler GPU users, can enjoy the speed boost of this pseudo-Y.F.C. 2 change, making games like `Metroid Prime Remastered` run much faster.
No more waiting for Samus to load her arm cannon.

We strongly advise users of old hardware without good Vulkan support to upgrade to something newer, as OpenGL is nearing its End of Life support soon-ish, but thanks to epicboy, those who can’t won’t suffer from terrible performance using the old but gold OpenGL API.

But that’s not all, epicboy also added an {{< gh-hovercard "9925" "optimization" >}} for `GL_SYNC_STATUS`, boosting performance slightly.
Your writer observes a 3-5% performance increase in `The Legend of Zelda: Breath of the Wild`, one of the few games that run faster on OpenGL with NVIDIA hardware, if you ignore shader stuttering.
...
This meant that programs (games, homebrew) would have to wait for a frame to show up on screen before starting to render, which in some cases is a bad example of feedback looping at its worst.
...
Here’s a couple of important ones for the Xenoblade fans, which Maide spent a lot of time working on.
...
We need more information to fully understand the issue, but for now, {{< gh-hovercard "9962" "ignoring" >}} colourspace transformations on sRGB samplers gives hardware-accurate rendering.
...
{{< imgs
	"./prime.png| Have you noticed how Samus lacks a lead indicator? She couldn’t spare a tiny bit more processing power into that? (Metroid Prime Remastered)"
  >}}
...
Let’s talk about the `Controller Applet`, which is responsible for asking the players to choose their input settings when the game requests it.
...
[Your writer](https://github.com/goldenx86) re-enabled LTO for GCC builds, following the changes made for Windows last time, but only applying the {{< gh-hovercard " optimizations" >}} to the core and video_core subprojects, giving Linux users a nice free performance boost.
...
While testing a crucial rendering fix for `The Legend of Zelda: Breath of the Wild` that we will discuss next month, Morph did some feature testing to investigate what makes Intel Windows drivers run like the Challenger project.
