...
Welcome yuz-ers, to the first progress report of 2021! We have quite a bit in store for you.
From kernel and CPU emulation improvements, to another major graphical rewrite done, here are the most important changes of January.
<!--more-->

## The Buffer Cache Rewrite

While we have a [dedicated article detailing what changes with the Buffer Cache Rewrite](https://yuzu-emu.org/entry/yuzu-bcr/) [(BCR)](https://github.com/yuzu-emu/yuzu/pull/5741), some changes and improvements have been happening since its introduction that require their own mention. Please read the dedicated article before continuing.

{{< single-title-imgs
    "BCR offers performance and rendering improvements (Xenoblade Chronicles Definitive Edition)"
    "./xcdebug.mp4"
    "./xcdefix.mp4"
  >}}
...
Assembly shaders are very limited and hard to work with due to their ancient origins. A problem faced during the BCR development is keeping compatibility with them while offering their advantage in shader build times to Nvidia users.
The first implementation at release [Rodrigo](https://github.com/ReinUsesLisp) tried, made use of `interops`, a feature that allows calling functions of one graphical API from another. In this particular case, using the new memory allocator of Vulkan from within an OpenGL context. 
...
All three main desktop/laptop vendors (Nvidia, AMD and Intel) now offer support for this extension in their latest driver.
AMD users updating from Radeon Software may need to allow `Optional` driver download support to get the, at the time of writing, corresponding driver version to have access to this valuable Vulkan extension.
...
What seems like just a small Quality of Life improvement, can have several considerations to make.
...
[toastUnlimited] continues his work on Linux’s `AppImages`.
...
`Super Mario Maker 2` experienced crashes at boot related to file system issues. By [fixing invalid buffer index errors,](https://github.com/yuzu-emu/yuzu/pull/5840) [Morph](https://github.com/Morph1984) took care of this.
...
In the past we blacklisted the `VK_EXT_extended_dynamic_state` Vulkan extension on all RDNA1 based GPUs on Windows, the drivers at the time were unstable when using this extension, crashing to the desktop.
...
Blue team’s turn. On the topic of blacklists, [Rodrigo](https://github.com/ReinUsesLisp) now had to [disable FP16 math support from Intel Windows Vulkan drivers](https://github.com/yuzu-emu/yuzu/pull/5798) to solve stability issues affecting `Astral Chain`. 
...
With this change we recommend to build your cache with assembly shaders enabled, but once the cache is complete or close to completion, switch to regular GLSL (disable assembly shaders). 
That way (after a long period of shader compilation at boot), stuttering will be completely avoided.
Thanks [Exzap](https://github.com/Exzap) from Cemu(https://cemu.info/) for the help!
...
While this doesn’t offer full VR support for PC compatible headsets yet, it allows users to do their own cardboard headsets a la Google, if they are inclined in handcraft work. 
Pro tip, stream to a phone or tablet.

{{< imgs
	"./vr.png| You will need a lot of cardboard if you want to use this in big displays (The Legend of Zelda: Breath of the Wild)"
  >}}
...
We reported this problem a long time ago to both vendors, it seems like OpenGL is not a priority on Windows. The Linux mesa team, on the other hand, [shows a *fantastic* response times](https://gitlab.freedesktop.org/mesa/mesa/-/issues/4034) to bug reports of any API.
...
The `ARM` architecture has a variety of special instructions (called `Memory Barrier Instructions`) that give the CPU a better control over how instructions are written in and from program memory.
...
These changes will also make it easier in the long term to add the code of future updates that Nintendo might do to their kernel, since the behaviour of this implementation will be closer to the hardware and the existing code won’t need many changes to accomodate for it.
...
Users experienced that some games may continue to send vibration signals, this is caused because the game never sends a 0 amplitude signal. [Morph](https://github.com/Morph1984) removes those bothersome moments for us by [adding a 1 second maximum time for any vibration.](https://github.com/yuzu-emu/yuzu/pull/5800)
...
[A wrong behaviour regarding player 1 was addressed](https://github.com/yuzu-emu/yuzu/pull/5743), which used to result in input changes not saving, specially in handheld mode.
